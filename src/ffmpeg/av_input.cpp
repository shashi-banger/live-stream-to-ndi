...
std::pair<std::wstring, std::wstring> protocol_split(const std::wstring& s)
{
    size_t pos;
...
Input::Input(const std::string& filename, boost::optional<bool> seekable)
    : filename_(filename)
    , seekable_(seekable)
{
    ...
    thread_ = boost::thread([=] {
        try {
            ...
            COMMON_LOG(error) << boost::current_exception_diagnostic_information();
        }
    });
}
...
void Input::internal_reset()
{
...
#endif
    auto           url_parts    = protocol_split(u16(filename_));
    ...
    if (seekable_) {
        COMMON_LOG(debug)  << "av_input[" + filename_ + "] Disabled seeking";
    ...
    for (auto& p : to_map(&options)) {
        COMMON_LOG(warning) << "av_input[" + filename_ + "]"
